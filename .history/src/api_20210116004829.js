...
export const deliveryManOnProgressDeliveries = async (status) => {
	let queryParams = ""; 
	for (let i = 0; i < status.length; i++) {
		if (i === 0) {
queryParams += ("status[]=" +status[i]);
		} else {
			queryParams += ("&status[]=" +status[i]);
		}
		
	}
	const resp = await fetch(process.env.REACT_APP_CHAP_CHAP_API_BASE_URL + "/deliveries?" + queryParams, {
		method: "GET",
		headers: {
			'Content-Type': 'application/json',
			'Accept': 'application/json',
...
export const deliveryInformation = async (code) => {
